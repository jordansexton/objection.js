...
class Relation {
  ...
  setMapping(mapping) {
    ...
    if (mapping.join.morph) {
      let joinMorph = this.parseReference(mapping.join.morph);

      if (!joinMorph.table || (joinMorph.columns.length !== 1)) {
        throw this.createError('join.morph must have format TableName.columnName. For example "SomeTable.type".');
      }

      this.morphCol = joinMorph.columns;

      if (joinMorph.table === this.ownerModelClass.tableName) {
        this.morphProp = this.propertyName(this.morphCol, this.ownerModelClass);
      } else if (joinMorph.table === this.relatedModelClass.tableName) {
        this.morphProp = this.propertyName(this.morphCol, this.relatedModelClass);
      } else {
        throw this.createError('join: `morph` must point to the owner or related model table.');
      }
    }
  ...
  clone() {
    ...
    relation.morphCol = this.morphCol;
    relation.morphProp = this.morphProp;
